...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def admission_slip_pdf(self, request, pk, *args, **kwargs):
        ...
        college_name = college_header_obj.college_name if college_header_obj else ''
        college_address = college_header_obj.address if college_header_obj else ''
...
class MarksViewSet(viewsets.ViewSet):
    ...
    def marksheet_report_pdf(self, request):
        ...
        # Grade evaluation helper for PASS / FAIL Result column
        from student.models import GradeSystem
        ...
        # Determine if column should be labeled 'Grade' or 'Marks(100)'
        has_grades = False
        ...
        # 2. Metadata Section Boxed Grid Table
        ...
        # 3. Marks Table Grid (Single 50/50 split table layout with Result column & gap)
        # Left Block 5 cols (262 pt) + Gap 1 col (11 pt) + Right Block 5 cols (262 pt) = 535 pt
    ...
    def progress_report_pdf(self, request):
        ...
        for s_idx, st in enumerate(students):
            ...
            def clean_pdf_text(val, split_slash=False):
                ...
                # Handle slash separated text for multi-line display when enabled
            ...
            # Marks Table
            # Case 1 (1 exam): S.No | Subject Code | Subject Name | Exam Name | Result Category
            # Case 2 (2 exams): S.No | Subject Code | Subject Name | Exam 1 | Result Category | Exam 2 | Result Category
            # Case 3 (3+ exams - "too much columns"): S.No | Subject Code | Subject Name | Exam 1 | Exam 2 | Exam 3 ...
            #   (Inside each exam column: Mark/Grade on line 1, Result Category on line 2 via multiline)
            if len(exams) <= 1:
                ...
                # 2 exams: Separate column for each exam's Result Category with multiline header support
                ...
                # 3+ exams ("too much columns"): Combine Marks/Grade and Result Category on the same column using multi-line
                num_exams = len(exams)
    ...
    def internal_exam_result_analysis_report_pdf(self, request):
        ...
        for subj_index, target_subj in enumerate(subjects):
            ...
            # 5. Statistics key-value summary table (Boxed grid table with multi-line paragraph support)
    ...
    def consolidated_exam_result_analysis_report_pdf(self, request):
        ...
        from timetable.models import ExamTimetable, ClassTimetable
        ...
        # Determine if selected exam is a University Exam or Internal Exam
        ...
        if is_university_exam:
            # Image 2 Format: Semester N / Cumulative breakdown grid table for University Exams
            ...
            # Image 1 Format: Category / Count table for Internal Exams
    ...
    def capa_report_pdf(self, request):
        ...
        # Fetch failed students for the selected subject
        ...
        # Date of exam from ExamTimetable
        from timetable.models import ExamTimetable, ClassTimetable
        ...
        # Effectiveness Verified By Section
